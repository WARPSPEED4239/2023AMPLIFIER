...
public class ArmSliderSetPosition extends CommandBase {
  ...
  public void execute() {
    ...
    if (mArm.getArmEncoderDeg() <= 110.0 && 2.0 < mSliderTargetPosition) {
      mSlider.setSliderSpeed(0.0);
    } else { // Slider can Move
      mSlider.setSliderPositionInches(mSliderTargetPosition);
    }
    
    // Arm can't move down if target is < 110 and if slider is not less or equal to 2 inches
    if (mSlider.getSliderEncoderPositionInches() >= 2.0 && 110.0 > mArmTargetPosition) {
